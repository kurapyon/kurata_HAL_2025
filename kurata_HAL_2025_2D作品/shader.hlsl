						 ...
						 out float4 outDiffuse		: SV_Target )
{
	...
	else if (Material.noTexSampling == 2)	// １だと色が混ぜられない1は使われている
	{	// 波動方程式
		...
		// g_Texture.Sample →インデックスコード(U/V座標)
		// g_SamplerState  →テクスチャから色をとってくる
		// inTexCoord	   →テクスチャの幅？
		color1 = g_Texture.Sample(g_SamplerState,  inTexCoord);
		color2 = g_Texture2.Sample(g_SamplerState, inTexCoord);

		// saturate→0以上は０に、１以上は１に
