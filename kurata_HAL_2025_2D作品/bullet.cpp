...
void UpdateBullet(void)
{
	...
	for (int i = 0; i < MAX_BULLET; i++)
	{
		if (g_Bullet[i].bUse)
		{
			...
			for (int j = 0; j < MAX_ENEMY; j++)		// 弾でiを使っているからエネミーはjを使う
			{
				// もしエネミーが生きていたら
				if (enemy[j].use == TRUE)
				{
					...
					{
						// 当たった時の処理
						enemy[j].use = FALSE;					//エネミーを消す
						g_Bullet[i].bUse = FALSE;				//弾を消す
						ReleaseShadow(enemy[j].shadowIdx);		// エネミーの影を消す
						ReleaseShadow(g_Bullet[i].shadowIdx);	// 弾の影を消す
