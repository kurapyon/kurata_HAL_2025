...
// プロトタイプ宣言
//*****************************************************************************
void ResetEnemyParticle(int i, int n);
...
static char* g_TexturName[TEXTURE_MAX] = {
	"data/TEXTURE/effect00.png",
	"data/TEXTURE/effect01.png",
	"data/TEXTURE/deathblow.png",
...
// エフェクト構造体
static ENEMY_EFFECT	effectWk[TEXTURE_MAX][ENEMY_NUM_EFFECTS];
...
HRESULT InitEnemyEffect(void)
{
	...
	{
		for (int i = 0; i < ENEMY_NUM_EFFECTS; i++)
		{
			effectWk[j][i].use = false;
			effectWk[j][i].elapsed = 0;

			for (int n = 0; n < ENEMY_NUM_PARTS; n++)
			{
				ResetEnemyParticle(i, n);
			}
		}
		...
		for (int i = 0; i < ENEMY_NUM_EFFECTS; i++)
		{
			if (effectWk[j][i].use)
			{
				...
				if (effectWk[j][i].isRemoveOnFinish == FALSE)
				{
					...
					//エフェクト作成レートの増加処理
					if (effectWk[j][i].effectCount < ENEMY_NUM_PARTS)
						effectWk[j][i].emitCounter++;

					//バッファに空きがあり、追加タイミングが来ていれば新たなエフェクトを追加する
					while ((effectWk[j][i].effectCount < ENEMY_NUM_PARTS) && (effectWk[j][i].emitCounter > EMISSION_RATE))
					{
						//全体追加フラグがONであれば空き領域全てに新たなエフェクトを追加する
						if (EMISSION_FULL)
							effectWk[j][i].effectCount = ENEMY_NUM_PARTS;
						else
				...
				while (effectIndex < effectWk[j][i].effectCount)
				{
					if (effectWk[j][i].pParticle[effectIndex].liveTime > 0)
					{
						...
						if (effectWk[j][i].isRemoveOnFinish) {
							...
							//バッファを初期化する
							ResetEnemyParticle(i, effectIndex);
							...
							if (effectIndex != (effectWk[j][i].effectCount - 1))
							{
								...
								ResetEnemyParticle(i, (effectWk[j][i].effectCount - 1));
							}
							effectWk[j][i].effectCount--;
						}
					}
...
void DrawEnemyEffect(void)
{
		...
		{
			if (effectWk[TEXTURE_EFFECT00][i].use == TRUE)		// このエフェクトが使われている？
			{		// Yes
				// テクスチャ設定
				GetDeviceContext()->PSSetShaderResources(0, 1, &g_Texture[TEXTURE_EFFECT00]);

				// レベルアップエフェクト
				for (int n = 0; n < effectWk[TEXTURE_EFFECT00][i].effectCount; n++)
				{
					if (effectWk[TEXTURE_EFFECT00][i].pParticle[n].isFinish == 0)
					{

						//バレットの位置やテクスチャー座標を反映
						float px = effectWk[TEXTURE_EFFECT00][i].pParticle[n].pos.x - scrollX;	// エフェクトの表示位置X
						float py = effectWk[TEXTURE_EFFECT00][i].pParticle[n].pos.y - scrollY;	// エフェクトの表示位置Y
						...
						float tx = (float)(effectWk[TEXTURE_EFFECT00][i].pParticle[n].PatternAnim % 10) * tw;	// テクスチャの左上X座標
						float ty = (float)(effectWk[TEXTURE_EFFECT00][i].pParticle[n].PatternAnim / 10) * th;	// テクスチャの左上Y座標
			...
			if (effectWk[TEXTURE_DEATHBLOW][i].use == TRUE)		// このエフェクトが使われている？
			{		// Yes
				// テクスチャ設定
				GetDeviceContext()->PSSetShaderResources(0, 1, &g_Texture[TEXTURE_DEATHBLOW]);

				// レベルアップエフェクト
				for (int n = 0; n < effectWk[TEXTURE_DEATHBLOW][i].effectCount; n++)
				{
					if (effectWk[TEXTURE_DEATHBLOW][i].pParticle[n].isFinish == 0)
					{

						//バレットの位置やテクスチャー座標を反映
						float px = effectWk[TEXTURE_DEATHBLOW][i].pParticle[n].pos.x - scrollX;	// エフェクトの表示位置X
						float py = effectWk[TEXTURE_DEATHBLOW][i].pParticle[n].pos.y - scrollY;	// エフェクトの表示位置Y
						...
						float tw = 1.0f / 5;	// テクスチャの幅
						float th = 1.0f / 5;	// テクスチャの高さ
						float tx = (float)(effectWk[TEXTURE_DEATHBLOW][i].pParticle[n].PatternAnim % 10) * tw;	// テクスチャの左上X座標
						float ty = (float)(effectWk[TEXTURE_DEATHBLOW][i].pParticle[n].PatternAnim / 10) * th;	// テクスチャの左上Y座標
