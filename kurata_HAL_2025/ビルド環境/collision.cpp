...
//=============================================================================
// 内積(dot)	符号（２つのベクトルの角度）の識別 鈍角なら負、直角なら０、鋭角なら正
//=============================================================================
...
// カプセルの当たり判定(子供の当たり判定で中心座標を求めて、それを使いapos,bposを求められる)
// apos , bpos: ボーンの座標	cpos: ぶつかる相手の座標	r1,r2: BC用の半径
BOOL CapsuleCollision(XMFLOAT3 apos, XMFLOAT3 bpos, XMFLOAT3 cpos, float r1, float r2)
{

	// ベクター型に変換
	XMVECTOR	a = XMLoadFloat3(&apos);
	XMVECTOR	b = XMLoadFloat3(&bpos);
	XMVECTOR	c = XMLoadFloat3(&cpos);

	// 当たり判定をとる球の中心座標(垂線を下したときの最短距離)を求める
	// 中心: b + t(a - b)
	float t = dotProduct(&(a - b), &(c - b)) / dotProduct(&(b - a), &(b - a));

	// 外側の球の内側で判定
	if (0 <= t && t <= 1)
	{
		// p1: 相手の座標		p2(中心):ボーンABに対して、p1から垂線をおろした最短距離
		XMFLOAT3 p1, p2;

		// XMFLOAT3へ変換
		XMStoreFloat3(&p1, c);
		XMStoreFloat3(&p2, (b + t * (a - b)));

		// 当たり判定
		return CollisionBC(p1, p2, r1, r2);

	}
	else // tが負か、１以上の時には両端の球と当たり判定
	{
		// p1: 相手の座標		p2,p3: 両端の球の中心座標
		XMFLOAT3 p1, p2, p3;
		...
		// 当たり判定		|（ビットor） = 0・0以外は１を返す。0・0なら０を返す
		return CollisionBC(p1, p2, r1, r2) | CollisionBC(p1, p3, r1, r2);
	}
}