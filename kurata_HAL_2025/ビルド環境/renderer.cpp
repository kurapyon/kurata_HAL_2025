...
// 背景色
static float g_ClearColor[4] = { 0.3f, 0.3f, 0.3f, 1.0f };
...
HRESULT InitRenderer(HINSTANCE hInstance, HWND hWnd, BOOL bWindow)
{
	...
	{
		// 画像を渡す
		ID3D11ShaderResourceView* g_Fractalnoise;

		// どのテクスチャを渡すか g_Fractalnoise ← この変数の初期化
	...
	g_ImmediateContext->VSSetConstantBuffers(8, 1, &g_DissolveBuffer);	// バーテックスシェーダー
	g_ImmediateContext->PSSetConstantBuffers(8, 1, &g_DissolveBuffer);	// ピクセルシェーダー
