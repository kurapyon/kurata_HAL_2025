...
void UpdatePad(void)
{
	...
	for ( i=0 ; i<padCount ; i++ ) 
	{
		...
		// 左スティック
		if (dijs.lY < 0)					padState[i] |= L_STICK_UP;
		//* y-axis (backward)
		if (dijs.lY > 0)					padState[i] |= L_STICK_DOWN;
		//* x-axis (left)
		if (dijs.lX < 0)					padState[i] |= L_STICK_LEFT;
		//* x-axis (right)
		if (dijs.lX > 0)					padState[i] |= L_STICK_RIGHT;

		// 右スティック
		if (dijs.lRz < 0)					padState[i] |= R_STICK_UP;
		//* y-axis (backward)
		if (dijs.lRz > 0)					padState[i] |= R_STICK_DOWN;
		//* x-axis (left)
		if (dijs.lZ < 0)					padState[i] |= R_STICK_LEFT;
		//* x-axis (right)
		if (dijs.lZ > 0)					padState[i] |= R_STICK_RIGHT;
...
DIJOYSTATE2 GetDeviceState(int num)
{
	// 初期化している
	HRESULT			result;
	DIJOYSTATE2		dijs{};

	result = pGamePad[num]->GetDeviceState(sizeof(DIJOYSTATE), &dijs);	// デバイス状態を読み取る
	if (result == DIERR_INPUTLOST || result == DIERR_NOTACQUIRED) {
		result = pGamePad[num]->Acquire();
		while (result == DIERR_INPUTLOST)
			result = pGamePad[num]->Acquire();
	}
